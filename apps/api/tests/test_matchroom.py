...
def test_connect_endpoint_returns_instance_id(client):
    response = client.post("/connect", json={"display_name": "  Ronnie  "})

    assert response.status_code == 200
    payload = response.json()
    assert payload["display_name"] == "Ronnie"
    assert payload["identity_type"] == "anonymous"
    assert payload["instance_id"]
    assert payload["matchroom_id"]


def test_connect_endpoint_respects_submitted_matchroom_id(client):
    response = client.post(
        "/connect",
        json={"display_name": "Judd", "matchroom_id": "  room42  "},
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["display_name"] == "Judd"
    assert payload["matchroom_id"] == "room42"
...
def test_arcade_match_lifecycle(client):
    ...
    with client.websocket_connect(f"/ws/room/?{p1_params}") as ws1:
        ...
        with client.websocket_connect(f"/ws/room/?{p2_params}") as ws2:
            ...
            # score_keeper defaults to "opp", so Player 2 logs Player 1's shot.
            ws2.send_text(
                json.dumps(
                    {
                        "player": "player1",
...
def test_successful_tournament_handshake(client, mock_rackup_client):
    ...
    with client.websocket_connect(f"/ws/room/?{params}") as ws:
        ...
        assert data["score_keeper"] == "opp"
        assert data["match"]["id"] == "999"
        assert data["match"]["name"] == "999"
        assert data["match"]["winning_condition"] == "Open frame"
        assert data["frame"]["points_remaining"] == 147
        assert data["frame"]["points_gap"] == 0
        assert data["frame"]["snookers_required"] == 0
        assert data["table"]["current_turn"] == "user_42"
        assert data["table"]["current_break"] == 0
        assert data["table"]["points_remaining"] == 147
        assert data["players"][0]["match_score"] == 0
        assert data["players"][0]["current_frame_score"] == 0
        assert data["players"][0]["highest_break"] is None
...
def test_score_keeper_opp_blocks_player_at_table(client):
    ...
    with client.websocket_connect(f"/ws/room/?{p1_params}") as ws1:
        ...
        with client.websocket_connect(f"/ws/room/?{p2_params}") as ws2:
            ...
            assert err["error"] == "You are not allowed to keep score in this turn."


def test_score_keeper_self_allows_player_at_table(client):
    p1_params = (
        "match_id=keeper_self_table&identity_type=anonymous&player_id=guest123&"
        "display_name=CasualRonnie&score_keeper=self"
    )
    p2_params = "match_id=keeper_self_table&identity_type=anonymous&player_id=guest789&display_name=CasualJudd"
...
def test_score_keeper_ref_blocks_both_players(client):
    p1_params = (
        "match_id=keeper_ref_table&identity_type=anonymous&player_id=guest123&"
        "display_name=CasualRonnie&score_keeper=ref"
    )
    p2_params = "match_id=keeper_ref_table&identity_type=anonymous&player_id=guest789&display_name=CasualJudd"
