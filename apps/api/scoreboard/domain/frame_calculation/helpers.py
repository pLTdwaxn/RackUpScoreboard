from scoreboard.domain.balls import COLOUR_BALLS
from scoreboard.domain.models.frame import Frame


def opponent_key(frame: Frame) -> str | None:
    for player_key in frame.scoring_state.scores:
        if player_key != frame.turn_state.current_turn:
            return player_key
    return None


def object_ball_equivalent_potted_balls(frame: Frame, potted_balls: tuple[str, ...]) -> tuple[str, ...]:
    nominated_colour = frame.table_state.free_ball_nominated_colour
    object_ball = frame.table_state.free_ball_object_ball
...
def scores_after_penalty(frame: Frame, points_awarded: int) -> dict[str, int]:
    ...
    if opponent is None:
        return dict(frame.scoring_state.scores)
    return scores_after_points(dict(frame.scoring_state.scores), opponent, points_awarded)
...
def remaining_colour_after(frame: Frame, colour: str) -> str | None:
    ...
    for next_colour in COLOUR_BALLS[index + 1 :]:
        if frame.table_state.colours_on_table.get(next_colour, False):
            return next_colour
    return None
